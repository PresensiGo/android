...
| [**getBatch**](BatchApi.md#getBatch) | **GET** /api/v1/batches/{batch_id} |  |
...
val batchId : kotlin.Int = 56 // kotlin.Int | batch id
try {
    val result : GetBatchRes = apiInstance.getBatch(batchId)
    println(result)
} catch (e: ClientException) {
    println("4xx response calling BatchApi#getBatch")
    e.printStackTrace()
} catch (e: ServerException) {
    println("5xx response calling BatchApi#getBatch")
    e.printStackTrace()
}
```
